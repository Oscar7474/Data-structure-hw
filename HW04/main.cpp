...
void addArrayBasedPoly(polynomialTerm a[], polynomialTerm b[], polynomialTerm d[])
{
	...
	while (a[pa].coef != 0 && b[pb].coef != 0) {
		if (a[pa].expo > b[pb].expo) {
			d[pd].expo = a[pa].expo;
			d[pd].coef = a[pa].coef;
			pd++;
			pa++;
		}
		else if (a[pa].expo < b[pb].expo) {
			d[pd].expo = b[pb].expo;
			d[pd].coef = b[pb].coef;
			pd++;
			pb++;
			...
			if (a[pa].coef + b[pb].coef != 0) {
				d[pd].expo = b[pb].expo;
				d[pd].coef = a[pa].coef + b[pb].coef;
				pd++;
			}
			pa++;
			pb++;
