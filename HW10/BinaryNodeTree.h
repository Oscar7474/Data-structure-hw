...
int BinaryNodeTree<ItemType>::getHighestFullLevelHelper(BinaryNode<ItemType>* subTreePtr) const
{
    if (subTreePtr==nullptr) {
        ...
        int leftHeight = getHighestFullLevelHelper(subTreePtr->getLeftChildPtr());
        int rightHeight = getHighestFullLevelHelper(subTreePtr->getRightChildPtr());
...
}  //圖怎麼畫  subTreePtr->getLeftChildPtr()得出來的到底是甚麼
...
int BinaryNodeTree<ItemType>::getMaximumHelper(BinaryNode<ItemType>* subTreePtr) const
{
    ...
    if (subTreePtr->getItem() > max(getMaximumHelper(subTreePtr->getLeftChildPtr()), getMaximumHelper(subTreePtr->getRightChildPtr()))) {
        return subTreePtr->getItem();
    }
    else {
        return max(getMaximumHelper(subTreePtr->getLeftChildPtr()), getMaximumHelper(subTreePtr->getRightChildPtr()));
    }
...
int BinaryNodeTree<ItemType>::getNumberOfNodesHelper(BinaryNode<ItemType>* subTreePtr) const
{
    ...
    else if (subTreePtr->getLeftChildPtr() && subTreePtr->getRightChildPtr() == nullptr) {
        return 1 + getNumberOfNodesHelper(subTreePtr->getLeftChildPtr());
    }
    else if (subTreePtr->getLeftChildPtr() == nullptr && subTreePtr->getRightChildPtr()) {
